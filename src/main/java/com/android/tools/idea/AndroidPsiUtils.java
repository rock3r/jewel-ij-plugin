...
public class AndroidPsiUtils {
    ...
    public static <T extends PsiElement> T getPsiParentOfType(@Nullable PsiElement element,
        ...
        for (PsiElement psiElement = element; psiElement != null; psiElement = psiElement.getParent()) {
            UElement uElement =
                    ApplicationManager.getApplication().getService(UastContext.class).convertElementWithParent(psiElement, UElement.class);
            if (uElement != null) {
     ...
     * @see com.intellij.psi.util.PsiTreeUtilKt#parentsOfType(PsiElement, Class<? extends T>)
     */
    @NotNull
